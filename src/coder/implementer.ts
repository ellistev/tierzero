...
const log = createLogger("implementer");
...
export class Implementer {
  ...
  async implement(ticket: Ticket): Promise<ImplementationResult> {
    ...
    try {
      ...
      log.info(`Sending ${files.length} files (${fileContext.length} chars) to ${this.model.modelName}...`);
      ...
      log.info(`Got response: ${response.length} chars`);
      log.info(`Response preview: ${response.slice(0, 500)}`);
      ...
      log.info(`Parsed ${plan.edits.length} edits, summary: ${plan.summary.slice(0, 200)}`);

      if (!plan.edits.length) {
        result.summary = plan.summary || "LLM produced no file edits — the change may require manual implementation.";
        log.warn(`No edits parsed from response. Full response:\n${response.slice(0, 2000)}`);
        result.durationMs = Date.now() - start;
        return result;
      }
