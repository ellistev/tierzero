...
import {newInject, getTypeName} from './utils/index.js';
...
/* README...
 ...
 these properties are UUIDs generated using uuidV4().

 a. to generate a uuidV4() value for 'someAggregateId' property on command, use

 app.post(..., commandActionHelper
 .handlerFor(...)
 .autoGenProperty('someAggregateId');

 b. if you want to control the generation of the value, instead of using uuidV4(),
  ...
  this.handlerFor = function(TAggregate, TCommand) {
    ...
    const handler = (req, res) => {
      ...
      validateCommandIds(command, autoGenPropertyBag)
        .then(() => {
          const aggregateId = command[aggregateIdProperty];
          return commandHandler(TAggregate, aggregateId, command);
        })
        .then(() => {
          res.json(command);
        })
    ...
    handler.autoGenProperty = function(propName, generatorFunction) {
      generatorFunction = generatorFunction || (() => uuidV4());
...
export default CommandActionHelper;
