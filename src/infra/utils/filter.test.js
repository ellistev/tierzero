...
describe('Filter', () => {
  const items = [
    ...
    {firstName: 'Roger+Mike', lastName: 'Bob', n: 5, phone: '5556078866'},
  ];
  // EQ
  testFilter({firstName: 'Roger'}, items, [0,2]);
  testFilter({firstName: {$eq: 'Roger'}}, items, [0,2]);
  // NEQ
  testFilter({firstName: {$neq: 'Roger'}}, items, [1,3,4]);
  // LT
  testFilter({n: {$lt: 3}}, items, [0,1]);
  // LTE
  testFilter({n: {$lte: 3}}, items, [0,1,2]);
  // GT
  testFilter({n: {$gt: 2}}, items, [2,3,4]);
  // GTE
  testFilter({n: {$gte: 2}}, items, [1,2,3,4]);
  // BETWEEN
  ...
  for (const specialChar of specialChars)
    testFilter({fieldName: {$ilike: '%roge' + specialChar + '%'}}, items, []);
  // SEARCH
  testSearch(['firstName', 'lastName', 'phone'], ['roger', 'bob'], items, [0,1,4]);
});
