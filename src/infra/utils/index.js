import {randomBytes} from "../runtime/crypto.js";
import path from "../runtime/path.js";

export function getTypeName(obj) {
  if (obj && obj.constructor && obj.constructor.type) {
...
export function merge(left, right) {
  const obj = deepClone(left);
  for (const k of Object.keys(right)) {
    if (typeof obj[k] === 'object' && typeof right[k] === 'object') {
      obj[k] = merge(obj[k], right[k]);
    } else {
      obj[k] = deepClone(right[k]);
    }
  }
  return obj;
}
...
export function cache(creator) {
  const _cache = {};
  return function(name = "__default") {
    if (!_cache[name]) {
      _cache[name] = creator(name);
    }
    return _cache[name];
  };
}
...
function _deepEqual(left, right, check_constructor, equals) {
  if (
  ...
  if (check_constructor && left.constructor !== right.constructor) return false;
  for (const k in left) {
...
export const deepEqual = (left, right) => _deepEqual(left, right, false, (a, b) => a == b);
export const deepStrictEqual = (left, right) => _deepEqual(left, right, true, (a, b) => a === b);
